...
module.exports = function ( opts ) {
	...
	module.handleSCSS = function ( srcPath, type ) {
		...
		var now = new Date(),
			...
			destPath = srcPath.replace( buildOptions.srcDir, buildOptions.distDir ).replace( 'scss/', 'css/' ).replace( '.scss', '.css' );
			...
			sass.convertSass( srcPath, destPath, buildOptions.css_minify, function ( err, result ) {
				...
				myFiles.forEach( function ( aFile ) {
					...
					if ( aFile.search( '.map' ) < 0 ) {
						...
						destPath = aFile.replace( buildOptions.distDir, '' );
	...
	module.handleJS = function ( srcPath, type ) {
		...
		var now = new Date(),
			fileExt = utils.get_ext( srcPath ),
			destPath = srcPath.replace( buildOptions.srcDir, buildOptions.distDir );
