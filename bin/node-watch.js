...
// Initialize watcher.
var watcher = chokidar.watch([buildOptions.srcDir], {
	persistent: true,
	ignored: ['.*'],
	ignoreInitial: true,
	followSymlinks: true,
...
var watcherSync = chokidar.watch([buildOptions.distDir], {
	persistent: true,
	ignored: ['.*'],
	ignoreInitial: true,
	followSymlinks: true,
...
watcher
	...
	.on('unlink', path => {
		...
		var now = new Date(),
			destPath = path.replace(buildOptions.srcDir, buildOptions.distDir);
...
watcher
	...
	.on('ready', () => {
		...
		log(('\nInitial scan of ' + (buildOptions.srcDir).yellow + ' complete.').bold + (' Ready for changes').green);
...
watcherSync
	.on('add', path => {
		...
		if (buildOptions.ftp && fileName.indexOf('.ds_store') < 0) {
			...
			var destPath = path.replace(buildOptions.distDir, ftproot);
	...
	.on('ready', () => {
		...
		log((buildOptions.ftp ? 'FTP: ' + ('ON').bold.green : 'FTP: ' + ('OFF').bold.red) + ('\n\nInitial scan of ' + (buildOptions.distDir).yellow + ' complete.').bold + (' Ready for changes').green);
